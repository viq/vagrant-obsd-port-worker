...
Vagrant.configure("2") do |config|
  ...
  config.vm.synced_folder "./srv", "/srv", id: "root", :nfs => true
  config.vm.synced_folder "~/Work/OpenBSD/ports", "/usr/ports", :nfs => true
  config.vm.synced_folder "~/Work/OpenBSD/pobj", "/usr/ports/pobj", :nfs => true
  config.vm.synced_folder "~/Work/OpenBSD/plist", "/usr/ports/plist", :nfs => true
